...
public class TGMap : MonoBehaviour {
	...
	void BuildElevation(){
		...
		StartLocation temp = new StartLocation ();
		temp.CreateStartLocation ();
		startTile.renderer.material = new Material(Shader.Find("Transparent/Diffuse"));
		startTile.renderer.material.color = new Color32(0,0,0,0);
	}
	IcySides checkSides(int x, int y){
		IcySides thingy = new IcySides();
	...
	void applyBlendedIceTexture(int x, int y, string iceTexture, string direction){
		if(elevationMap[x,y].FindChild("pCube4")){
			Transform thingything=elevationMap[x,y].FindChild("pCube4");
			thingything.renderer.material.mainTexture = Resources.Load(iceTexture) as Texture;
			thingything.renderer.material.color = new Color32(0,0,0,0);
			if(direction.Equals("top")){
				thingything.rotation = Quaternion.Euler(new Vector3(0,0,0));
			}
			else if(direction.Equals("bottom")){
				thingything.rotation = Quaternion.Euler(new Vector3(0,180,0));
			}
			else if(direction.Equals ("right")){
				thingything.rotation = Quaternion.Euler(new Vector3(0,90,0));
			}
			else if(direction.Equals ("left")){
				thingything.rotation = Quaternion.Euler(new Vector3(0,270,0));
			}
			...
			//thingything.transform.rotation = direction;
			//thingything.transform.TransformDirection(new Vector3(0.0f,50.0f,0.0f));
		} else{
			elevationMap[x,y].renderer.material.mainTexture = Resources.Load(iceTexture) as Texture;
