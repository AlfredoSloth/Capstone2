...
public class AStar {
	...
	class Node {
		...
		public Node parent;
		public TDTile cell;
		public Node (int x, int y, float G, float F, float H, Node parent, TDTile c) {
			this.x = x;
			this.y = y;
			this.G = G;
			this.H = H;
	...
	public void FindPath(TDTile startCell, TDTile goalCell, TDTile[,] map, bool targetCellMustBeFree) {
		this.map = map;
		this.mapWidth = map.GetLength(0);
		this.mapHeight = map.GetLength(1);
		
		start = new Node((int)startCell.getXPos(), (int)startCell.getYPos(), 0, 0, 0, null, startCell);
		end = new Node((int)goalCell.getXPos(), (int)goalCell.getYPos(), 0, 0, 0, null, goalCell);
		openList.Add(start);
	...
	public List<TDTile> CellsFromPath() {
	...
	float MovementCost(Node a, Node b) {
		return map [b.x, b.y].getMovementCost ();
	}
