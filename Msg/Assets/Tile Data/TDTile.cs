...
public class TDTile{
	...
	private void setResources(){
		...
		if (resourceType > 1 && resourceType < 500) {
			TileResource none = new TileResource("none", 0);
			resources.Add(none);
		} else if (resourceType >= 500 && resourceType < 750) {
			TileResource building = new TileResource("building", Random.Range(10,20));
			resources.Add(building);
			int secondResource = Random.Range (1,100);
			if(secondResource>=1&&secondResource<=25){
				TileResource water = new TileResource("water", Random.Range(5,10));
				resources.Add(water);
			}
		} else if (resourceType >= 750 && resourceType <= 1000) {
			TileResource water = new TileResource("water", Random.Range(5,10));
			resources.Add(water);
			int secondResource = Random.Range (1,100);
			if(secondResource>=1&&secondResource<=25){
